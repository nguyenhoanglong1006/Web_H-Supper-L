...
public class NhanVienImpl implements NhanVienDAO {
	...
	public void deleteNV(int id) {
		Session currentSession = sessionFactory.getCurrentSession();
		String hql = "UPDATE hl_web.nhanvien set trangThai = 'Đã nghỉ' WHERE id = :employee_id";
	...
	public void updateNV(NhanVien nhanVien, int id,String emailcu) {
		Session currentSession = sessionFactory.getCurrentSession();
		String hql = "UPDATE hl_web.nhanvien set tenNV = '" + nhanVien.getTenNV() + "',email='" + nhanVien.getEmail()
		...
		String sql2 = "UPDATE hl_web_security.users set username = '" + nhanVien.getEmail()
	...
	public NhanVien getNVEmail(String email) {
		Session currentSession = sessionFactory.getCurrentSession();
		// now retrieve/read from database using the primary key
		Query<NhanVien> theQuery = currentSession.createNativeQuery("select * from hl_web.NhanVien where  email='"+email+"'",
				NhanVien.class);
	...
	public void updatePass(int id, String pass) {
		...
		Session currentSession = sessionFactory.getCurrentSession();
		NhanVien nv = currentSession.get(NhanVien.class, id);
		String sql2 = "UPDATE hl_web_security.users set  password= '" + pass
		+ "' WHERE username = :email";
		Query query3 = currentSession.createNativeQuery(sql2);
